...
class BaseJWTVerifier():
    ...
    def parse_token(self, token):
        ...
        Return:
            tuple (headers, claims, signature)
        """
    ...
    async def verify_access_token(self, token, claims_to_verify=('iss', 'aud', 'exp')):
        ...
        try:
            ...
            self.verify_signature(token, signature, okta_jwk)
    ...
    async def verify_id_token(self, token, claims_to_verify=('iss', 'exp'), nonce=None):
        ...
        try:
            ...
            jwk = await self.get_jwk(headers['kid'])
    ...
    def verify_signature(self, token, signature, jwk_key):
...
class IDTokenVerifier():
    ...
    async def verify(self, token, claims_to_verify=('exp',), nonce=None):   # 'iss', 
...
class Validator:
    ...
    async def validate_token(token):
        try:
            ...
            header = json.loads(base64.b64decode(header).decode('utf-8'))
            payload = json.loads(base64.b64decode(payload).decode('utf-8'))
            jwt_verifier = IDTokenVerifier(issuer='http://identity-provider:8090/',  # user_info['iss'],
                                           client_id=payload['aud'])  #'4wxbKNnJbudt1QLpXuU2cq6Si08d479y')
